...
void CRpcWrapper::excute(const string& service, const char* param, int paramlen, char* result, int& reslen) throw (CException)
{
    ...
    try
    {
        ...
        // 远程过程调用
		size_t rspLenth=reslen;
		int iRet = trpc_call(m_pRpc, service.c_str(), param, paramlen, result, (size_t*)&rspLenth);    
		reslen = rspLenth;
 ...
 *                  paramlen     参数长度
 */
void CRpcWrapper::excute_asyn(const string& service, const char* param, int paramlen) throw (CException)
{
    struct timeval begin, end;
    ...
    try
    {
        ...
        gettimeofday(&begin, NULL);
         ...
         // 远程过程异步调用
        int iRet = trpc_asyn_call(m_pRpc, service.c_str(), param, paramlen);

        // 获取调用结束时间
        gettimeofday(&end, NULL);

        // 打印日志
        gPtrAppLog->debug("rpc_asyn|%s|%s|itg_deal_server|%s|%s|%d||%s|%dms|%d|%s |",
                                        SRC_IP, HOST_IP, SERVICE, m_strHost.c_str(), m_iPort, service.c_str(),
                                        (end.tv_sec  - begin.tv_sec) * 1000 + (end.tv_usec- begin.tv_usec)/1000,
                                        iRet, param);
