 ...
 */
CMySQL::CMySQL(const char* host, const char* user, const char* passwd, const int port, int iOverTime, const char*  charset, const char * db_name, int iIdleTime)
    :
    m_strHost(host),
    m_iPort(port),
    m_strUser(user),
    ...
    m_connFlag(false),
    m_isTrans(false),
    m_idle_time(iIdleTime)
{
}

CMySQL::CMySQL(const std::string& host, const std::string& user, const std::string& passwd, const int port, int iOverTime, const std::string& charset, const std::string& db_name, int iIdleTime)
    :
    m_strHost(host),
    m_iPort(port),
    m_strUser(user),
...
void  CMySQL::Connect() throw(CException)
{
    ...
    if (!mysql_real_connect(m_SqlHandle, m_strHost.c_str(), m_strUser.c_str(), m_strPasswd.c_str(), NULL, 0, NULL, 0))
    {
...
void CMySQL::Set_idle_time()
{
    ...
    std::string sql = std::string("set wait_timeout=") + toString<int>(m_idle_time);

    // set wait_timeout
    Query(sql.c_str(), sql.length());

    sql = std::string("set interactive_timeout=") + toString<int>(m_idle_time);
...
/**
 * Description: 获取最后插入ID
 * Return:      失败抛出异常
 */
unsigned long long CMySQL::InsertID() throw(CException)
{
	return mysql_insert_id(m_SqlHandle);
}
